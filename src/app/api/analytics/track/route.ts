...
export async function POST(req: NextRequest) {
  try {
    ...
    const event: AnalyticsEvent = {
      ...
      metadata: data.metadata ? JSON.stringify(data.metadata).replace(/,/g, ';') : '' // Replace commas in JSON
    };
    ...
    const csvLine = [
      ...
      event.userId,
      event.sessionId,
      `"${event.metadata}"` // Properly quote the metadata
    ].join(',') + '\n';
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Read file and manually parse it to handle the JSON in metadata
